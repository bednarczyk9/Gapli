...
# Add project root to path to ensure libs are importable
...
WHOLESALERS_FILE = os.path.join(PROJECT_ROOT, "archive/Recorded/hurtownie_allegro.xlsx")
...
class GapliAPIClient:
    ...
    def get_products(self, parser_id, min_price, max_price, min_stock, account_id=None):
        """Fetches products from a specific wholesaler based on criteria."""
        url = f"{self.base_url}/products"
        all_products = []
        offset = 0
        limit = 100 # Reduced limit for better stability
        ...
        while True:
            ...
            try:
                ...
                if response.status_code in [500, 502, 503, 504]:
                    logger.warning(f"Server error ({response.status_code}) during fetch. Retrying in 30s...")
                    time.sleep(30)
                    continue
                response.raise_for_status()
                data = response.json()
        ...
        for i in range(0, len(skus), batch_size):
            ...
            while retries > 0:
                ...
                try:
                    ...
                    if response.status_code in [500, 502, 503, 504]:
                        logger.warning(f"Server error ({response.status_code}). Retrying in 30s ({retries} retries left)...")
                        time.sleep(30)
                        retries -= 1
                        continue

                    if response.status_code != 200:
                        logger.error(f"Failed to send batch: {response.status_code} {response.text[:500]}")
                        # On maintenance page (502 usually), response.text is huge HTML. Truncating for logs.
                        if "Trwa aktualizacja systemu" in response.text:
                            logger.info("Maintenance detected. Waiting 60s...")
                            time.sleep(60)
                            retries -= 1
                            continue
                        break
...
def get_token_via_browser():
    ...
    try:
        ...
        logger.error(f"Required libraries (ChromeManager, GapliClient) not found in libs/: {e}")
        return None
    ...
    try:
        with sync_playwright() as p:
            ...
            if client.login(username, password):
                # Trigger some API calls to capture token
...
def main():
    ...
    if not api_key:
        # Check for GAPLI_TOKEN from other scripts as fallback
        api_key = os.environ.get("GAPLI_TOKEN")
        
    if not api_key:
        logger.info("Gapli_Apikey not found in environment. Attempting browser-based capture...")
    ...
    if not wholesalers_to_process:
        logger.warning(f"No wholesalers found to process in {WHOLESALERS_FILE}.")
        return
...
if __name__ == "__main__":
    try:
        ...
        logger.info("python-dotenv not installed, skipping load_dotenv()")
    main()
