...
# Gemini API via direct REST (to avoid SDK model version issues)
...
def process_and_fix_all_customizations():
    ...
    for item in items:
        ...
        # Check if cleanup is needed (quick pre-check)
        if any(tag in desc.lower() for tag in ['<i>', '<em>', '<span>', '<div>', '<br', 'style=']):
