...
def fetch_actionable_errors(account_id):
    url = "https://gapli.com/api/products-manager/allegro/products"
    all_failed = []
    for status in ["ERROR", "VALIDATION_ERROR"]:
        params = {"konto_allegro_id": account_id, "status": status, "limit": 100, "page": 1, "mode": "full"}
        resp = requests.get(url, headers=GAPLI_HEADERS, params=params)
        if resp.status_code == 200:
            products = resp.json().get("products", [])
            # ZERO STOCK FILTER
            active = [p for p in products if p.get("gapli_product_stock_quantity", 0) > 0]
            all_failed.extend(active)
            logger.info(f"Status {status}: Found {len(products)} products, {len(active)} are active (stock > 0).")
    return all_failed
...
def nuke_and_repair_product(p, allegro_token):
    sku = p.get("sku")
    p_id = p.get("id")
    account_id = p.get("konto_allegro_id")
    cat_id = p.get("allegro_catalog_category_id")
    
    logger.info(f"--- Processing SKU: {sku} (ID: {p_id}, Acc: {account_id}) ---")
    ...
    for item in raw_params:
        ...
        if k and v: existing_params[str(k)] = str(v)
    ...
    # 3. Permanent Delete
    if not permanent_delete(p_id, account_id):
        logger.error("Failed to permanently delete product. Skipping.")
        return False
    logger.info("Product permanently deleted from integration.")
    
    # 4. Re-send
    time.sleep(2) # Give DB a moment
    if not send_to_allegro(sku, account_id):
        logger.error("Failed to re-send product.")
        return False
    logger.info("Product successfully re-sent to Allegro!")
...
def run_pipeline(shop_name):
    ...
    products = fetch_actionable_errors(account_id)
    logger.info(f"Total actionable products found for {shop_name}: {len(products)}")
    
    for p in products:
        nuke_and_repair_product(p, allegro_token)
        time.sleep(3) # Throttle

if __name__ == "__main__":
    # Example execution: python pipeline/nuke_and_repair_errors.py hit_bazar
    import sys
