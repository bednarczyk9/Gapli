...
# Gemini API via direct REST - Try without specific version in path
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if 'candidates' in data and data['candidates']:
                ...
                logger.error(f"No candidates. Resp: {data}")
                return None
        else:
            # If gemini-pro also fails, try one more time with a very generic one
            logger.warning(f"gemini-pro failed ({resp.status_code}), trying gemini-1.0-pro")
            url_alt = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-1.0-pro:generateContent?key={GEMINI_API_KEY}"
            resp = requests.post(url_alt, json=payload)
            if resp.status_code == 200:
...
def process_and_fix_all_customizations():
    ...
    for item in items:
        ...
        # We process if there are ANY non-Allegro tags or if it's generally messy
        needs_ai = any(tag in desc.lower() for tag in ['<i>', '<em>', '<span>', '<div>', '<br', 'style=', '<h3', '<h4', '<font'])
        ...
        if needs_ai:
            ...
            logger.info(f"  SKU {sku} looks clean. Skipping.")
