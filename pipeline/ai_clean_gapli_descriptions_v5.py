...
def rewrite_description_with_ai(name, description):
    ...
    try:
        ...
        # Fallback to model list to see what's actually there
        logger.warning(f"Primary endpoint failed ({resp.status_code}), fetching model list...")
        list_url = f"https://generativelanguage.googleapis.com/v1beta/models?key={GEMINI_API_KEY}"
        models_resp = requests.get(list_url)
        if models_resp.status_code == 200:
             models = [m['name'] for m in models_resp.json().get('models', [])]
             logger.info(f"Available models: {models}")
             # Try first available that supports generateContent
             for m_name in models:
                 if 'gemini' in m_name:
                     logger.info(f"Trying fallback model: {m_name}")
                     try_url = f"https://generativelanguage.googleapis.com/v1beta/{m_name}:generateContent?key={GEMINI_API_KEY}"
                     r = requests.post(try_url, json=payload)
