...
def update_customization():
    print(f"Updating customization for SKU {SKU}...")
    # Explicitly setting Kolor, Odcień and manufacturer color name
    payload = {
...
def fix():
    # 1. Fetch all records for this SKU
    ...
    print(f"Found {len(products)} product integration records.")
    ...
    for p in products:
        ...
        # Check stock - only process if stock > 0
        stock = p.get("gapli_product_stock_quantity", 0)
        if stock is None or int(stock) <= 0:
            print(f"Skipping ID {p_id} (Acc {acc_id}) due to zero stock.")
            continue
